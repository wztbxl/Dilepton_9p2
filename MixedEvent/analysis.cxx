...
const Int_t mCenBins = 1; //used for do mixed centrality by centrality
...
int main(int argc, char** argv)
{
  ...
  for (int iCent = 0; iCent < mCenBins; iCent++)
  {
    for (int iVz = 0; iVz < mVzBins; iVz++)
    {
      ...
      {
		hMixPollFullFlag->SetBinContent(iCent+1,iVz+1,iEve+1,(int)bufferFullFlag[iCent][iVz][iEve]);
		hnEvtsPerMixPoll->SetBinContent(iCent+1,iVz+1,iEve+1,nEventsInBuffer[iCent][iVz][iEve]);
...
void makeTags() // using for additional photon electron cut M_ee < 0.055 GeV
{
	...
	for(Int_t iBufferEvent=0;iBufferEvent<nEventsInBuffer[cenBufferPointer][vzBufferPointer][eveBufferPointer];iBufferEvent++){
		...
		for(Int_t i=0;i<current_nEPlus;i++){
			for(Int_t j=0;j<buffer_nEMinus[cenBufferPointer][vzBufferPointer][eveBufferPointer][iBufferEvent];j++){
				...
				if ( abs( current_ePlus_CellID[i]-buffer_eMinus_CellID[cenBufferPointer][vzBufferPointer][eveBufferPointer][iBufferEvent][j] ) == 0 ) continue;//remove the pair with same TOF cell ID
...
void bookHistograms()
{
	...
	//for QA 
	hMixPollFullFlag = new TH3D("hMixPollFullFlag","hMixPollFullFlag;mCentraliy;mVzBin;mEvtPlaneBin",mCenBins,0,mCenBins,mVzBins,0,mVzBins,mEveBins,0,mEveBins);
	hnEvtsPerMixPoll = new TH3D("hnEvtsPerMixPoll","hnEvtsPerMixPoll;mCentraliy;mVzBin;mEvtPlaneBin",mCenBins,0,mCenBins,mVzBins,0,mVzBins,mEveBins,0,mEveBins);
}
//=======================================================================================
void writeHistograms(char* outFile)
{
