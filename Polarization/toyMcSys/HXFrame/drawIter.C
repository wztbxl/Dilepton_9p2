...
double pPar[3][nthetaphi] = {{-0.021, -0.331, -0.169, 0.117}, {-0.059, 0.125, 0.254, -0.012}, {-0.187, 0.070, 0.842, 0.089}};//theta; phi; inv vs. pT bins
double pParErr[3][nthetaphi] = {{0.128, 0.277, 0.218, 0.342}, {0.066, 0.272, 0.134, 0.24}, {0.208, 1.02, 0.855, 0.805}};//stat. err of theta; phi; inv vs. pT bins
...
int drawIter(int nIteration = 8, int savePDF=1, int saveHisto=1)
{
    ...
    for(int t=0; t<3; t++){
        for(int p=0; p<nPtBins; p++){
            ...
            if(t<2){
                if(p==0) {
                    hJpsiPolParErr[p][t] = new TH1D(Form("Iter%d_pt%d_%s_Err", nIteration, p, typeName[t]),Form("; Err(#lambda_{%s}); ", polTitle[t]), 120, 0, 0.2);
                    hJpsiPolPar[p][t] = new TH1D(Form("Iter%d_pt%d_%s_mean", nIteration, p, typeName[t]),Form("; #lambda_{%s}; ", polTitle[t]), 400, -2, 2);
                }
                else {
                    hJpsiPolParErr[p][t] = new TH1D(Form("Iter%d_pt%d_%s_Err", nIteration, p, typeName[t]),Form("; Err(#lambda_{%s}); ", polTitle[t]), 100, 0, 1.0);
                    hJpsiPolPar[p][t] = new TH1D(Form("Iter%d_pt%d_%s_mean", nIteration, p, typeName[t]),Form("; #lambda_{%s}; ", polTitle[t]), 200, -2, 2);
                }
            }
            else if(t==2) {
                if(p==0) hJpsiPolPar[p][t] = new TH1D(Form("Iter%d_pt%d_%s_mean", nIteration, p, typeName[t]),Form("; #lambda_{%s}; ", polTitle[t]), 200, -2, 2);
                else hJpsiPolPar[p][t] = new TH1D(Form("Iter%d_pt%d_%s_mean", nIteration, p, typeName[t]),Form("; #lambda_{%s}; ", polTitle[t]), 200, -4, 6);
                
                if(p==0) hJpsiPolParErr[p][t] = new TH1D(Form("Iter%d_pt%d_%s_Err", nIteration, p, typeName[t]),Form("; Err(#lambda_{%s}); ", polTitle[t]), 200, 0, 1);
                else hJpsiPolParErr[p][t] = new TH1D(Form("Iter%d_pt%d_%s_Err", nIteration, p, typeName[t]),Form("; Err(#lambda_{%s}); ", polTitle[t]), 50, 0, 4);
            }
    ...
    for(int t=0; t<3; t++){
        for(int p=0; p<nPtBins; p++){
            ...
            if(t==2 && p>0)fgPar[p][t]->SetRange(mean-2.5*rms, mean+2.*rms);
            ...
            if(t==2 && p==1 && nIteration!=8) fgParErr[p][t]->SetRange(mean-1.5*rms, mean+1.*rms);
            else if(t==2 && p>1 && nIteration!=8) fgParErr[p][t]->SetRange(mean-1.5*rms, mean+1*rms);
        ...
        if(!savePDF) continue;
        //c[t]->SaveAs(Form("Toy_getMean_HX_%s.pdf", typeName[t]));
        if(t==0)c[t]->Print(Form("getMean_HX_iter%d.pdf(", nIteration),Form("Title: %s",typeName[t]));
        else if(t==2) c[t]->Print(Form("getMean_HX_iter%d.pdf)", nIteration),Form("Title: %s",typeName[t]));
        else c[t]->Print(Form("getMean_HX_iter%d.pdf", nIteration),Form("Title: %s",typeName[t]));
    }
    ...
    double x[1], y[1];
       ...
       if(nIteration==8 && saveHisto){
           TFile *fout2 = new TFile(Form("HX_compare_%d.root", nIteration),"recreate");
           fout2->cd();
           for(int p=0; p<nPtBins; p++){
               for(int t=0; t<3; t++){
    ...
    for(int t=0; t<3; t++){
        ...
        leg[t]->Draw("same");
        if(t==0)drawLatex(0.15, 0.8, "Helicity frame", 22, 0.05, 2);
    }
    
    if(savePDF)c1->SaveAs(Form("HXFinal_iter%d.pdf", nIteration));
    
    if(saveHisto){
        TFile *fout = new TFile("HX_real.root","update");
        fout->cd();
        int nIter;
        if(nIteration==10) nIter = 0;
        else if(nIteration>0 && nIteration<10) nIter = nIteration;
        for(int t=0; t<3; t++){
            gPolParDiff[t]->SetName(Form("gPolParHXIter%d%s", nIter, typeName[t]));
            gPolParErrDiff[t]->SetName(Form("gPolParErrHXIter%d%s", nIter, typeName[t]));
            gPolParDiff[t]->Write();
